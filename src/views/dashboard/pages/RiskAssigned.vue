<template>
  ...
  >
    ...
    <base-material-card
      ...
      icon="mdi-table"
      inline
      :title="`Liste des projets assignés :  ${totalItems} projets`"
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <v-list-item
                      :disabled="item.financial_infos.amount_requested < 500000 || item.analysis.risk_manager.agent !== agent.id"
                      @click="selectAgent(item.analysis._id, item.sectors[0]._id)"
                    >
                      <v-list-item-icon>
                        <v-icon
                          small
...
<script>
  ...
  export default {
    ...
    async beforeCreate () {
      ...
      await this.$store.dispatch('analysis/fetchAnalysisByRiskManager', { riskManager: this.agent.id, page: this.page, item: this.perPage })
    },
    /*
    async created () {
      await this.$store.dispatch('analysis/fetchAnalysisBySectorOwner', { riskManager: this.agent.id, page: this.page, item: this.perPage })
    },
    */
    methods: {
