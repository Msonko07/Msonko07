      ...
      <v-simple-table>
        ...
        <tbody>
          ...
          >
            ...
            <td>
              <v-menu
                v-if="agents && item.analysis && item.analysis.review_owner && item.analysis.review_owner.agent"
                bottom
                min-width="250px"
                rounded
                offset-y
                  ...
                  <v-list-item-content class="justify-center">
                    <div class="mx-auto text-center">
                      <v-avatar
                        color="secondary"
                      >
                        <span class="white--text subtitle-1">{{ agents.find(a => a.id === item.analysis.review_owner.agent).initials }}</span>
                      </v-avatar>
                  ...
                  >
                    ...
                    <v-list-item
                      :disabled="item.financial_infos.amount_requested < 500000 ||
                        !['directionOwner', 'divisionOwner', 'portfolioOwner'].includes(agent.role) ||
                        agent.board.tag !== 'di'
                      "
                    ...
                    <v-list-item
                      :disabled="agent.role !== 'directionOwner' ||
                        agent.board.tag !== 'di' ||
                        !item.analysis.review_owner.agent ||
                        item.analysis.review_owner.state === 'waiting' ||
                        !item.analysis.portfolio_owner.agent ||
                        item.analysis.portfolio_owner.state === 'waiting' ||
                        !item.analysis.review.agent ||
                        item.analysis.review.state === 'waiting'
                      "
      ...
      <v-card>
        ...
        <v-card-text>
          <v-select
            ...
            class="ma-0 pa-0"
            :items="agents.filter(item => ['directionOwner', 'divisionOwner', 'portfolioOwner'].includes(item.role) && item.board.tag === 'di')"
            item-text="fullName"
            item-value="id"
            dense
            label="Responsable"
...
<script>
  ...
  export default {
    ...
    async beforeCreate () {
      ...
      await this.$store.dispatch('analysis/fetchAnalysisToBeReviewed', { page: this.page, item: this.perPage })
    },
    methods: {
      ...
      affectProject () {
        this.$store.dispatch(
          'analysis/assignReview',
          {
            project: this.toAnalyse.project,
            review_owner: this.agent.id,
            portfolio_owner: this.toAnalyse.agent,
