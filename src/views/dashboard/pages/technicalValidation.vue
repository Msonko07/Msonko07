      ...
      <v-simple-table>
        ...
        <tbody>
          ...
          <td>{{ (item.recipient && item.recipient.firstname) ? item.recipient.firstname : '' }}</td>-->
            ...
            <td>
              {{ item.sectors.map(sub => sub.name).join(' / ') }}
            </td>
            <td>
              {{ item.subsectors.map(sub => sub.name).join(' / ') }}
            </td>
            <td class="text-right">
                ...
                <v-list-item-content class="justify-center">
                  <div class="mx-auto text-center">
                    <v-avatar
                      color="secondary"
                    >
                      <span class="white--text subtitle-1">{{ agents.find(a => a.id === item.analysis.sector_owner.agent).initials }}</span>
                    </v-avatar>
                ...
                <v-list-item-content class="justify-center">
                  <div class="mx-auto text-center">
                    <v-avatar
                      color="secondary"
                    >
                      <span class="white--text subtitle-1">{{ agents.find(a => a.id === item.analysis.analysis.agent).initials }}</span>
                    </v-avatar>
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <v-list-item
                      :disabled="item.financial_infos.amount_requested < 500000 || !['directionOwner', 'divisionOwner', 'programOwner'].includes(agent.role)"
                      @click="selectAgent(item.id, item.analysis._id)"
                    >
                      <v-list-item-icon>
                        <v-icon
                          small
      ...
      <v-card>
        ...
        <v-card-text>
          <v-select
            ...
            class="ma-0 pa-0"
            :items="agents.filter(item => ['directionOwner', 'divisionOwner', 'programOwner'].includes(item.role))"
            item-text="fullName"
            item-value="id"
            dense
            label="Responsable"
      ...
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn
            color="primary"
            text
            :disabled="!advice.text || !advice.type || !advice.analysis"
...
<script>
  ...
  export default {
    ...
    async beforeCreate () {
      ...
      await this.$store.dispatch('projects/fetchProjects', { page: this.page, item: this.perPage })
    },
    /*
    async created () {
      // window.console.log({ owner: this.agent.id })
      await this.$store.dispatch('analysis/fetchAnalysisBySectorOwner', { sectorOwner: this.agent.id, page: this.page, item: this.perPage })
    },
    */
    methods: {
      ...
      selectAgent (projectId, analysisId) {
